...
fn handle_read(manifest: Manifest, field: Option<String>) -> Result<String, Error> {
    if let Some(field) = field {
        let val = manifest.get(field.as_str())?;
...
fn handle_write(
    ...
    if let Some(field) = field {
        manifest.set(field.as_str(), value.as_str())?;
...
fn main() -> Result<(), Error> {
    ...
    let value = match opts.set {
        Some(v) => handle_write(&mut manifest, opts.path, opts.field, v),
        None => handle_read(manifest, opts.field),
    }?;
